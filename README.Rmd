

<!-- README.md is generated from README.Rmd. Please edit that file -->

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "man/figures/README-",
  out.width = "100%"
)
```

# comunicacion

<!-- badges: start -->
[![R-CMD-check](https://github.com/d4t4tur/comunicacion/workflows/R-CMD-check/badge.svg)](https://github.com/d4t4tur/comunicacion/actions)
<!-- badges: end -->

La librería `{comunicacion}` tiene por objetivo facilitar el trabajo de visualización de la información con la que se trabaja en la [Dirección Nacional de Mercados y Estadística](https://www.yvera.tur.ar/estadistica/) (DNMyE-MINTURDEP).


## Installation



Podés instalar la versión en desarrollo [GitHub](https://github.com/) con:

``` r
# install.packages("devtools")
devtools::install_github("d4t4tur/comunicacion")
```



